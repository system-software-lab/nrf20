...
def main():
    ...
    for i in range(nn):
        ...
        #output_logits = res[output_blob][0][0]
        #print('result:',output_logits,'\n')
        #print(numpy.shape(res[output_blob]))
        #for i in range(32):
        #    print(res[output_blob][i][0],'\n')
        
        outputs=outputs+numpy.array(res[output_blob]).flatten().tolist()

        #print(a.index(max(a)), y_test[r])
        #if a.index(max(a))!=y_test[r]:
        #        count=count+1
        st=end
        end=end+hop
    ...
    numpy.savetxt('saveincept.txt',outputs,fmt='%1.4f')
