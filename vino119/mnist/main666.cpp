//mnist_openvino
//main666.cpp
//measure energy consumption_2_time+accuracy/apply sign on new main.cpp
...
void tofp16(const short int* i,float* arr){
	...
	for(k=0;k<10;k++)
	{
		...
		//if(sign==1) sign=-1;
		//else sign=1;
		//if(expo<0) temp2=a[abs(expo)];
		//else temp2=1<<expo;
		//arr[k]=pow(2,expo)*mant*pow(-1,sign+2);
		sign=(!sign-1)+!sign;
		arr[k]=pow(2,expo)*mant*sign;
		//arr[k]=temp2*mant*sign;
	}
}
...
st=clock();
	...
	std::cout<<"result: "<<count<<"/"<<10-count<<std::endl;
	//input_data_close
	delete[] arr;
	delete[] label_data; 
	delete[] res;	
